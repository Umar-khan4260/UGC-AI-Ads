...
export const createProject = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const response: any = await ai.models.generateContent({
      model,
      contents: [img1base62, img2base62, prompt],
...
export const createVideo = async (req: Request, res: Response) => {
  ...
  //deduct credits for video geenration
  await prisma.user
    .update({
      where: { id: userId },
  ...
  try {
    ...
    if (project.generatedVideo) {
      return res.status(404).json({ message: "Video already geerated" });
    }
    ...
    const prompt = `make the person showcase the product which is ${project.productName} ${project.productDescription && `Product Description: ${project.productDescription}`}`;
    ...
    const imageBytes: any = await axios.get(project.generatedImage, {
    ...
    //create the images directory of it doesnot exsist
    fs.mkdirSync("videos", { recursive: true });

    if (!operation.response.generatedVideos) {
      throw new Error(operation.response.raiMediaFilteredResponses[0]);
    }
    ...
    //remove video from the disk
    fs.unlinkSync(filePath);
    res.json({
      message: "Video geenrated Successfully",
