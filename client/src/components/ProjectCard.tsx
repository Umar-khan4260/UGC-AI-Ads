...
const ProjectCard = ({
  ...
  const togglePublish = async(projectId: string) => {
    console.log("toggle publish",projectId);
  }

  return(
  <div key={gen.id} className="mb-4 break-inside-avoid">
    <div className="bd-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-white/20 transition group">
      {/* preview */}
      <div className={`${gen?.aspectRatio === "9:16" ? 'aspect-9/16' : 'aspect-video'} relative overflow-hidden`}>
        ...
        <div className="absolute left-3 top-3 flex gap-3 items-center">
            ...
            {gen.isPublished && (
                <span className="text-xs px-2 py-1 bg-green-600/30 rounded-full text-green-300">Published</span>
            )}
        </div>

        {/* action menu ofr my generations only */}
        {!forCommunity && (
          <div
          ...
          className="absolute top-3 right-3 sm:opacity-0 group-hover:opacity-100 transition flex items-center gap-2">
            ...
            <div className="flex flex-col items-end w-32 text-sm">
              <ul className={`text-xs ${menuOpen ? 'block' : 'hidden'} overflow-hidden right-0 peer-focus:block hover:block w-40 bg-black/50 backdrop-blur text-white border border-gray-500/50 rounded-lg shadow-md mt-2 z-10 py-1`}>
              ...
              {gen.generatedVideo && <a href='#' download className="flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer">
                ...
                {(gen.generatedImage || gen.generatedVideo) && (
                  <button onClick={()=>navigator.share({url:gen.generatedImage || gen.generatedVideo, title:gen.productName, text:gen.productDescription})} className="w-full flex gap-2 items-center px-4 py-2 hover:bg-black/10 cursor-pointer">
        ...
        {/* source images */}
        <div className="absolute right-3 v-3 bottom-3">
            <img src={gen.uploadedImages[0]} alt="product" className="w-16 h016 object-cover rounded-full animate-float" />
             <img src={gen.uploadedImages[1]} alt="model" className="w-16 h016 object-cover rounded-full animate-float -ml-8" style={{animationDelay:'3s'}} />
        </div>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="text-right">
            <div className="mt-2 flex flex-col items-end gap-1">
              <span className="test-xs px-2 py-1 bg-white/5 rounded-full">Aspect: {gen.aspectRatio}</span>
            </div>
        ...
        {!forCommunity && (
          <div className="mt-4 grid grid-cols-2 gap-3">
            <GhostButton className="text-xs justify-center" onClick={()=> {navigate(`/result/${gen.id}`); window.scrollTo(0,0);}}>
              View Details
            </GhostButton>
