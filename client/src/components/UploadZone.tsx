...
const UploadZone = ({lable, file, onClear, onChange}:UploadZoneProps) => {
  return (
    <div className="relative group">
        <div className={`relative h-64 rounded-2xl border-2 border-dashed 
            transition-all duration-300 flex flex-col items-center justify-center 
            bg-white/2 p-6 ${file? 'border-voilate-600/50 bg-voilate-500/5':'border-white/10 hover:border-voilate-500/30 hover:bg-white/5'}`}>
              {file?(
                ...
                <div className="absolute bottom-4 left-4 right-4 bg-black/50 backdrop-blur-md p-3 rounded-lg border border-white/10">
                  ...
                  <>
                  <div className="w-16 h-15 rounded-full bg-white/5 flex items-center justify-center md-4 group-hover:scale-110 transition-transform duration-300">
                     <UploadIcon className="w-8 h-8 text-gray-400 group-hover:text-voilate-400 transition-colors "/>
                  </div>
                  <h3 className="text-lg font-semibold md-2">{lable}</h3>
